...
namespace UnityEngine.UI
{
        ...
        ILayoutElement, ILayoutGroup
    {
        ...
        //  ########### Yox ########### 
        public bool noNegativeNumbers = true;
        //  ########### Yox ########### 
        ...
        {
        //  ########### Yox ########### 
            RefreshCellsAt(itemTypeStart);
        }

        private void RefreshCellsAt(int index)
        {
            if (Application.isPlaying && this.isActiveAndEnabled)
            {
                int lineStartOffset = index % contentConstraintCount;
                itemTypeStart = index - lineStartOffset;
                ...
                for (int i = 0; i < content.childCount; i++)
                {
                    ...
                    //{
                    dataSource.ProvideData(content.GetChild(i).GetComponent<LoopScrollDataReceiver>(), itemTypeEnd);
                    itemTypeEnd++;
                    //}
                    //else
                    //{
        ...
        public void JumpTo(int index, bool forceJump = false, float speed = 2000f)
        {
            if(!forceJump && Math.Abs(index - itemTypeStart) < 200)
            {
                SrollToCell(index, speed);
            }
            else
            {
                RefreshCellsAt(index);
            }
            SetDirty();
        }
        //  ########### Yox ########### 
        ...
        protected float NewItemAtStart()
        {
            ...
            {
                if (totalCount >= 0 || noNegativeNumbers)
                {
                    return 0;
                }
            }
        ...
        {            
            ...
            //  ########### Yox ########### 
            dataSource.ProvideData(nextItem.GetComponent<LoopScrollDataReceiver>(), itemIdx);
            //  ########### Yox ########### 
            return nextItem;
        }
        //==========LoopScrollRect==========
